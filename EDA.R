...
library(writexl)
scores_un_renamed %>% write_xlsx("Data/scores_un_renamed.xlsx")
...
library(skimr)
skim(scores_un_renamed_19up) # Ok, so I found out that reading_understanding, math_basic, reading_drop and math_adv_score are not captured for this age group. Which makes total sense. 
skim(scores_un_renamed_15_18) # Lower rates of completion are the participated_tvet, is_working_field (which is dep in tvet* check), and received_certification 
skim(scores_un_renamed_12_14) # Same as 15_18
skim(scores_un_renamed_9_11) # No data for not_s_w_t , participated_tvet, is_enrolled_tvet, is_working, is_working_field, 
# received_certification, communication_score, thinking_score
skim(scores_un_renamed_6_8) # Solely has data for is_enrolled_school. And 99.7% of them are.
...
scores_un_renamed_19up_imputed_selected <- scores_un_renamed_19up_imputed %>%
    select("is_enrolled_school",
           ...
           # "reading_drop",
           "is_working_field",
           "received_certification") #,
# "communication_score",
# "thinking_score",
# "math_adv_score")


# I should think more carefully, but as of now, I will chose to impute 0 for when it is NA
# Should correct it for the thinking and communication score
scores_un_renamed_19up_imputed_selected[is.na(scores_un_renamed_19up_imputed_selected)] <- 0


data_corr <- scores_un_renamed_19up_imputed_selected %>%
    correlate(is_working)

data_corr %>% plot_correlation_funnel()


data_corr_field <- scores_un_renamed_19up_imputed_selected %>%
    correlate(is_working_field)

data_corr_field %>% plot_correlation_funnel()

# Correlation plots
scores_un_renamed_19up_imputed %>% corrmorant::corrmorant()
